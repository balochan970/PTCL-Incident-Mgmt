...
export default function FiberPathsSection({ paths, onSave, onUpdate, onDelete, onEdit }: FiberPathsSectionProps) {
  ...
  const totalPages = Math.ceil(sortedPaths.length / entriesPerPage);
        ...
        <>
          <div className="table-container">
            <table ref={tableRef}>
              <thead>
                <tr>
                  <th onClick={() => handleSort('linkName')} className="cursor-pointer">
                    <div className="flex items-center gap-2">
                      Link Name
                      <ArrowUpDown className="h-4 w-4" />
                    </div>
                  </th>
                  <th onClick={() => handleSort('nodeA')} className="cursor-pointer">
                    <div className="flex items-center gap-2">
                      Node A
                      <ArrowUpDown className="h-4 w-4" />
                    </div>
                  </th>
                  <th onClick={() => handleSort('nodeASectionLength')} className="cursor-pointer">
                    <div className="flex items-center gap-2">
                      Node A Length (kms)
                      <ArrowUpDown className="h-4 w-4" />
                    </div>
                  </th>
                  <th onClick={() => handleSort('nodeASectionOwner')} className="cursor-pointer">
                    <div className="flex items-center gap-2">
                      Node A Owner
                      <ArrowUpDown className="h-4 w-4" />
                    </div>
                  </th>
                  <th onClick={() => handleSort('nodeB')} className="cursor-pointer">
                    <div className="flex items-center gap-2">
                      Node B
                      <ArrowUpDown className="h-4 w-4" />
                    </div>
                  </th>
                  <th onClick={() => handleSort('nodeBSectionLength')} className="cursor-pointer">
                    <div className="flex items-center gap-2">
                      Node B Length (kms)
                      <ArrowUpDown className="h-4 w-4" />
                    </div>
                  </th>
                  <th onClick={() => handleSort('nodeBSectionOwner')} className="cursor-pointer">
                    <div className="flex items-center gap-2">
                      Node B Owner
                      <ArrowUpDown className="h-4 w-4" />
                    </div>
          ...
          <div className="pagination-container flex items-center justify-between p-4 border-t">
            ...
            <div className="flex items-center gap-2">
              <div className="text-sm text-gray-600">
                Showing {startIndex + 1} to {Math.min(endIndex, paths.length)} of {paths.length} entries
              </div>
